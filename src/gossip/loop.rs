    ...
    loop {
        ...
        if round.is_multiple_of(rounds_per_hour) {
            ...
            match akamu_crdt::db::persist_crdt(&state.db, &purged_snapshot).await {
                ...
                Err(e) => {
                    tracing::error!(err = %e, "gossip: persist after tombstone purge failed — in-memory state unchanged");
                }
            }
        }
        ...
        let mut prepared_peers: Vec<PreparedPeer> = Vec::new();
        {
            ...
            for peer_url in &all_peers {
                ...
                let crdt_bytes = match GossipEnvelope::encode_crdt(&payload_crdt) {
                    Ok(b) => b,
                    Err(e) => {
                        tracing::error!(peer = %peer_url, err = %e, "gossip: CBOR encode CRDT failed");
                        continue;
                    }
                };
                ...
                let nonce = match random_nonce() {
                    Ok(n) => n,
                    Err(e) => {
                        tracing::error!(peer = %peer_url, err = %e, "gossip: nonce generation failed");
                        continue;
                    }
                };
                ...
                let envelope_bytes = match envelope.encode() {
                    Ok(b) => b,
                    Err(e) => {
                        tracing::error!(peer = %peer_url, err = %e, "gossip: CBOR encode envelope failed");
                        continue;
                    }
                };
        ...
        for pp in prepared_peers {
            let signed_body = match sign_and_seal(
                ...
                Ok(b) => b,
                Err(e) => {
                    tracing::error!(peer = %pp.url, err = %e, "gossip: sign_and_seal failed");
                    continue;
                }
            };
            ...
            let handle: JoinHandle<Result<Vec<u8>, ()>> = tokio::spawn(async move {
                ...
                match resp {
                    Err(e) => {
                        tracing::warn!(peer = %display_url, err = %e, "gossip: request failed");
                        Err(())
                    }
        ...
        for pt in peer_tasks {
            ...
            let plaintext = match verify_and_open(
                ...
                Ok(p) => p,
                Err(e) => {
                    tracing::warn!(peer = %pt.url, err = %e, "gossip: verify_and_open response failed");
                    peer_last_gen.remove(pt.url.as_str());
                    peer_response_gen.remove(pt.url.as_str());
                    continue;
                }
            ...
            let peer_envelope = match GossipEnvelope::decode(&plaintext) {
                Ok(e) => e,
                Err(e) => {
                    tracing::warn!(peer = %pt.url, err = %e, "gossip: CBOR decode peer envelope failed");
                    peer_last_gen.remove(pt.url.as_str());
                    peer_response_gen.remove(pt.url.as_str());
                    continue;
                }
            ...
            let peer_crdt = match peer_envelope.decode_crdt() {
                Ok(c) => c,
                Err(e) => {
                    tracing::warn!(peer = %pt.url, err = %e, "gossip: CBOR decode peer CRDT failed");
                    peer_last_gen.remove(pt.url.as_str());
                    peer_response_gen.remove(pt.url.as_str());
                    continue;
                }
            ...
            if let Err(e) = akamu_crdt::db::persist_crdt(&state.db, &crdt_snapshot).await {
                tracing::error!(peer = %pt.url, err = %e, "gossip: persist after merge failed");
                continue;
            }
