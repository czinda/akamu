...
#[error("invalid listen address '{input}': {reason}")]
...
mod tests {
    ...
    fn parse_listen_target_uses_config_when_env_absent() {
        std::env::remove_var("_AKAMU_TEST_LISTEN_ABSENT");
        let t = parse_listen_target("127.0.0.1:9000", "_AKAMU_TEST_LISTEN_ABSENT").unwrap();
        assert!(matches!(t, ListenTarget::Tcp(addr) if addr.port() == 9000));
    }

    #[test]
    fn parse_listen_target_env_overrides_config() {
        std::env::set_var("_AKAMU_TEST_LISTEN_OVERRIDE", "/run/test.sock");
        let t = parse_listen_target("127.0.0.1:9000", "_AKAMU_TEST_LISTEN_OVERRIDE").unwrap();
        std::env::remove_var("_AKAMU_TEST_LISTEN_OVERRIDE");
    ...
    fn parse_listen_target_non_unicode_env_is_error() {
        ...
        let key = "_AKAMU_TEST_LISTEN_NONUTF8";
