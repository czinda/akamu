...
--
-- The UPDATE backfill is idempotent: if the migration is interrupted and
-- re-run, rows already set correctly are updated to the same value (no-op).
...
CREATE INDEX IF NOT EXISTS idx_orders_ca_account ON orders(ca_id, account_id);
CREATE INDEX IF NOT EXISTS idx_authzs_ca_id ON authorizations(ca_id);
