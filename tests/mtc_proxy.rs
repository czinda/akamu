//! Integration test for the `/acme/mtc/*` reverse-proxy middleware
//! (`src/routes/mtc_proxy.rs`) — a node that is not the elected MTC writer
//! for a CA must transparently relay reads to the writer rather than
//! answering from its own frozen local log.
//!
//! Node/identity setup mirrors `tests/gossip_bootstrap.rs`; MTC-enabled
//! `AppState` construction mirrors `tests/gossip_mtc_forward.rs`.
...
/// Build an MTC-enabled `AppState` for `identity`, with no gossip loop —
/// this test drives CRDT state directly rather than waiting on real gossip
/// propagation.
async fn build_mtc_node(identity: &NodeIdentity, base_url: &str) -> NodeHandle {
    ...
    let config = Arc::new(Config {
        ...
        cas: vec![CaConfig {
            ...
            crl_url: None,
            ocsp_url: None,
            common_name: "MTC Proxy Test CA".into(),
...
/// A non-writer node's local MTC log is frozen wherever it was left; a
/// `GET /acme/mtc/tree-size` against it must return the elected writer's
/// live tree size (via the reverse-proxy), not its own stale local value.
#[tokio::test]
async fn non_writer_node_proxies_mtc_reads_to_the_writer() {
    ...
    let writer = build_mtc_node(&id_w, &url_w).await;
    let non_writer = build_mtc_node(&id_n, &url_n).await;

    let now = akamu::util::unix_now();

    // Writer claims "default"'s election in its own CRDT.
    assert!(writer
        .state
        .crdt
        .write()
    ...
    // Give the writer's log real, distinguishable content: append one more
    // leaf beyond the seeded null_entry so its tree_size (2) differs from a
    // pristine log's (1).
    {
    ...
    assert!(
        writer_tree_size > 1,
        "writer's log must have grown past the seeded null_entry"
    );

    // Sanity: the non-writer's own (never-appended-to) log must NOT already
    // report the same value — otherwise this test couldn't distinguish
    // "served locally" from "proxied to the writer".
    ...
    .unwrap();
    assert_ne!(
        non_writer_local_tree_size, writer_tree_size,
        "test setup: local and writer tree sizes must differ to prove proxying happened"
    );

    // Simulate what gossip merge would eventually deliver to the non-writer:
    // knowledge of the writer's election claim and its gossip_url.
    {
    ...
    let resp = client
        .get(format!("{url_n}/acme/mtc/tree-size"))
        .send()
        .await
        .unwrap();
    ...
    assert_eq!(
        body["treeSize"].as_u64().unwrap(),
        writer_tree_size,
        "non-writer must proxy to the writer's tree size, not serve its own stale local log"
    );
}
